...
[ "$("$bin_one/hiveforge" version)" = "0.7.0" ]
"$bin_one/hiveforge" onboard >/dev/null

export_root="$TMP/export/unps-hiveforge-v0.7.0"
mkdir -p "$export_root"
for item in \
  00_README \
  03_SKILLS \
...
archive="$TMP/unps-hiveforge-v0.7.0.tar.gz"
tar -czf "$archive" -C "$TMP/export" unps-hiveforge-v0.7.0
mkdir -p "$TMP/imported"
tar -xzf "$archive" -C "$TMP/imported"

import_root="$TMP/imported/unps-hiveforge-v0.7.0"
...
[ "$("$bin_two/hiveforge" version)" = "0.7.0" ]
